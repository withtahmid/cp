...
signed main(){
    ...
    dbg(__init__());precompute();
    bool multiple_test_case = true;
    int tc = 1; if(multiple_test_case){read(tc);}
    for(int i = 1; i <= tc; ++i){dbg(__case__(i));solve(i);}
    dbg(__elapsed__());
    return 0;
}

...
struct Node{
    ...
    void merge(Node a, Node b){
        ...
        this -> factors.clear();
        this -> factors.insert(all(a.factors));
        this -> factors.insert(all(b.factors));
    }
...
void solve(const int case_no){
    ...
    //     while( l <= i and len(st.query(l, i).factors) > k){
    //         ++l;
    //     }

    //     Node node = st.query(l, i);
        
    //     if(len(node.factors) == k and node.gcd == 1 ){            
    //         res = max(res, i - l + 1);
    //     }

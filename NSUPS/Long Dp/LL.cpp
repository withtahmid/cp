...
ll fn(int prev, int l, int r){
    cerr << l <<" " << r << " " << prev <<endl;
    if(abs(r-l) == 1){
        if(prev == 1){
            return abs(v[r] - v[l]);
        }else{
            return -1LL * abs(v[r] - v[l]);
        }
    }
    if(dp[l][r][prev] != -1LL){
    ...
    if(prev == 1){
        ll res1  = v[l] + fn(0, l+1, r);
        ll res2 = v[r] + fn(0, l, r-1);
        dp[l][r][prev] = max(res1, res2);
        DP[l][r][prev].push_back(dp[l][r][prev]);
        return dp[l][r][prev];
    }
    else{
        ll res1 = v[l] + fn(1, l+1, r);
        ll res2 = v[r] + fn(1, l, r-1);

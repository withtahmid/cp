...
pair<int, int64_t> dp[maxDigit][2][2][9 * 20];
...
int64_t fn(const string& L, const string& R, int pos, int LT, int RT, int sum = 0, int64_t num = 0){
    if(pos == -1){
    ...
    for(int i = lo; i <= hi; ++i){
        res += fn(L, R, pos - 1,  (LT&&(i==lo)),(RT&&(i==hi)), sum + i, num * 10  + i);
    }
    return res;
}

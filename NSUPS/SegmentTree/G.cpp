...
private:
    ...
    void buildTree(const vector < T > & arr, int stIndex, int left, int right) {
        if (left == right) {
            return nodes[stIndex].assignLeaf(arr[left], left);
        }
...
struct LSTNode {
    ...
    void mergeNode(LSTNode& LC, LSTNode& RC) {
        this -> x = LC.x + RC.x, this -> y = LC.y + RC.y;
    }

    void assignLeaf(int64_t val, int indx){
        this -> lazy = 0, this -> x  = 100, this -> y = (indx + 1) * 100;
    }
};

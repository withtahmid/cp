...
void precompute(){}
void brutforce(const auto& v){local(
    int n = len(v);
    int N = (1 << n);
    
    auto beauty = [](auto& x) -> int{
        if((len(x) < 3) or (x[1] < x[0]) or (x[len(x) -1] > x[len(x) - 2])){
            return -1;
        }
        int cnt = 0;
        for(int i = 1; i + 1 < len(x); ++i){
    ...
    for(int i = 0; i < N; ++i){
        ...
        for(int mask = 0; mask < n; ++mask){
            if((i >> mask)&1){
                temp.push_back(v[mask]);
            }
        }
        dbg(temp);
        bst.insert(beauty(temp));
    }
    dbg(bst);
);}
...
int fn(int i, int inc, const auto& v){
    // if(i >= )

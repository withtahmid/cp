...
void solve([[maybe_unused]] const int case_no){
    vector<int>a(scan<int>());
    read(a);
    vector<int>b(scan<int>());
    read(b);
    vector<int>c(scan<int>());
    read(c);
    int n = len(a), m = len(b), o = len(c);
    vector<ll>bigger(m + 1, 0LL);
    for(int i = min(m, o) - 1; i >= 0; --i){
        int cnt = (c.end() - lower_bound(c.begin() + i, c.end(), b[i]));
        bigger[i + 1] = cnt;
    }
    for(int i = 2; i < len(bigger); ++i){
        bigger[i] += bigger[i - 1];
    }
    ll res = 0LL;
    for(int i = 0; i < min(m, n); ++i){
        int idx = (lower_bound(b.begin() + i, b.end(), a[i]) - b.begin());
        if(idx < m){
            res += (bigger.back() - bigger[idx]);
        }
    }
    println(res);
}

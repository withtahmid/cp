...
#define endl "\n"
#define ll long long
#define I_am_starting_in_the_name_of_Allah \
...
bool isSmaller(double a, double b) { return b - a > EPS; }
...
int main() {
  ...
  /// Lower Bound
  *ptr =
      lower_bound(a, a + n, x);  // if x exist then it will return x but it is
                                 // not it returns it shortest elder value;
  *ptr = lower_bound(a + 4, a + n, x);
  ...
  ll Nezuko(ll n) {
    ...
    if (n != 1) return -1;
    ll Zenitsu = 0;
    while (!DemonArt.empty()) {
      Zenitsu = Zenitsu * 10 + DemonArt.top();
      DemonArt.pop();
    }
    return Zenitsu;
  }
...
for (size_t i = 1; i <= 1e6; i++) lg[i] = lg[i - 1] + log10(i);
